...
public class search extends Base {
    ...
    public void i_validate_the_products_are_coming() {
        ...
        try {
            extentTest.info("Validating if products are displayed");
            homepage = new Homepage(driver);
            homepage.nocheckproductcoming();
            extentTest.pass("Validated that no products are coming");
            
        } catch (Exception e) {
        	screenshot = new Scrennshot(driver);
            String screenshotPath = screenshot.getScreenshot("search_results_fail",driver);
            extentTest.pass("Screenshot taken of search results",
            		MediaEntityBuilder.createScreenCaptureFromPath(screenshotPath).build());
            
            extentTest.log(Status.FAIL, "Failed to validate if products are coming: " + e.getMessage());
        }
    }
